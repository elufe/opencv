...
void fill2(cv::Mat black) {
	...
	while ((row / 2 + a * m) >=0 && (col / 2 + b * m) <= col)
		m++;
	xs = row / 2 + a * (m-1);
	ys = col / 2 + b * (m - 1);
	...
	for (i = xs, j = ys; i < row / 2; i-=a,j-=b) {
//		cout << black.at<uchar>(j, i) << "\n";
		if (black.at<uchar>(j, i) == 0)
			black.at<uchar>(j - b, i - a) = 0;
	}
//	cout << i << " " << j;
}
